/**
 * Exercise 01 — Inventory report (review, solution)
 *
 * The literal union makes `"backordered"` impossible; the tuple type on
 * `counts` tells the compiler index 0/1/2 always exist, so `counts[0]++`
 * is safe even under `noUncheckedIndexedAccess`.
 */
...
const tally = (items: Item[]): [inStock: number, low: number, out: number] => {
